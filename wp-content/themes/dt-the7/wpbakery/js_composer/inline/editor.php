...
Class NewVisualComposer extends WPBakeryVisualComposerAbstract {
    ...
    function assetUrl($asset_path) {
        ...
        return preg_replace('/\s/', '%20', get_template_directory_uri() . '/wpbakery/js_composer/inline/' . $this->settings['assets_dir'] .'/'. $asset_path);
    }
    ...
    function renderEditButton() {
        if($this->showButton()) {
            return ' <a href="'.self::getInlineUrl().'" id="vc-load-inline-editor" class="vc-inline-link">'.__('Edit with Visual Composer', LANGUAGE_ZONE).'</a>';
        }
    }
...
/*
        echo "\n\n------------------------------------------------\n\n";
        var_dump( $shortcodes[0]['string'], do_shortcode(stripslashes($shortcodes[0]['string'])) );
        echo "\n\n------------------------------------------------\n\n";
*/
        $this->enqueueRequired();
    ...
    public function buildInlineEditForm() {
        ...
        if(sizeof($elements) > 1) {
            ...
            foreach($elements as $element) {
                echo '<h3 class="handler">'.ucfirst($element['name']).' '.__('settings', LANGUAGE_ZONE).'</h3>';
    ...
    function parseShortcodesString($content ,$is_container = false, $parent_id = false) {
        ...
        // echo "\n\n-----------------------before armaggedon-------------------------\n\n";
        // var_dump($content, $found[2], $is_container, $parent_id);
        // echo "\n\n------------------------------------------------\n\n";
